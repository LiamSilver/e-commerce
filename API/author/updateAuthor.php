...
$author = json_decode($json);

// Verifica se o ID do autor foi fornecido
if (!isset($_GET['id'])) {
    http_response_code(400); // bad request
    echo '{ "message": "O ID do autor deve ser fornecido." }';
    exit();
}

// Obtém o ID do autor a ser atualizado
$id = $_GET['id'];
if (!is_numeric($id)) {
    http_response_code(400); // bad request
...
$authorDAO->update($id, $author->nome);

// Retorna o autor atualizado como JSON
